...
#Best product would be profile#16
...
```{r }
# rating preference data table
r_table <- data.frame()
for (i in unique(conjointData$ID)) {
  for (j in unique(conjointData$profile)) {
    r_table[i,j] <- conjointData[conjointData$ID==i & conjointData$profile==j,]$ratings
  }
}

colnames(r_table) <- unique(conjointData$profile)
r_table
...
sim_market <- simScenarios(scens,r_table)[,c(5,4,12,14,16,13,15,7,8)]
```
